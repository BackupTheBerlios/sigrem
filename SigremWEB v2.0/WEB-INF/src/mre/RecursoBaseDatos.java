...
public class RecursoBaseDatos 
{
	...
	public static ArrayList dameRecursos(String codigo,DataSource dataSource) 
	{
		Recurso recurso = null;
		...
		{	conn = dataSource.getConnection();
			stmt = conn.createStatement();
			stmt2 = conn.createStatement();
			rs = stmt.executeQuery("select * from recursos where codigoMulta=\'"+ codigo + "'");
			while (rs.next()) 
			{	recurso = new Recurso();
	...
	public static ArrayList dameRecursosAbogado(String codigo,DataSource dataSource) 
	{
		Recurso recurso = null;
	    ArrayList recursos = new ArrayList();
	    Connection conn = null;
		...
		{	conn = dataSource.getConnection();
			stmt = conn.createStatement();
			rs = stmt.executeQuery("select * from recursos,asignadorecursoempleado where recursos.codigoRecurso=asignadorecursoempleado.codigoRecurso and codigoEmpleado=\'"+ codigo + "'");
			while (rs.next()) 
			{	recurso = new Recurso();
				recurso.setCodigoRecurso(rs.getString("codigoRecurso"));
				recurso.setFechaEmision(rs.getString("fechaEmision"));
	...
	public static void eliminaRecurso(String codigo, DataSource dataSource) throws Exception 
	{
		...
		{	conn = dataSource.getConnection();
			...
			sqlString=new StringBuffer("delete from asignadorecursoempleado where codigoRecurso='" + codigo + "'");
			stmt.execute(sqlString.toString());
		}
		finally {
			if ( rs != null ) {
	...
	public static void insertaRecurso(Recurso recurso, DataSource dataSource) throws Exception 
	{
		...
		{	conn = dataSource.getConnection();
			...
			stmt.execute(sqlString.toString());
			rs2=stmt2.executeQuery("select codigo,perfil from empleados where codigo='"+recurso.getAbogado()+"'");
      		if (rs2.next())
      		{	String perfil=rs2.getString("perfil");
	...
	public static void modificaRecurso(Recurso recurso, DataSource dataSource) throws Exception 
	{
		...
		{	conn = dataSource.getConnection();
      		...
      		if (rs2.next())
      		{	rs3=stmt2.executeQuery("select * from asignadorecursoempleado where codigoRecurso='"+recurso.getCodigoRecurso()+"'");
      			if (rs3.next())
      			{	sqlString=new StringBuffer("delete from asignadorecursoempleado where codigoRecurso='" + recurso.getCodigoRecurso() + "'");
					stmt3.execute(sqlString.toString());
      			}
      			sqlString=new StringBuffer("insert into asignadorecursoempleado ");
      			sqlString.append("values (\""+recurso.getCodigoRecurso()+ "\", ");
